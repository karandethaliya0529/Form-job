...
function validatePreferredLocation(){
    const loc1 =document.getElementById('prefLoc1').value.trim();
    const loc2 =document.getElementById('prefLoc2').value.trim();
    const loc3 =document.getElementById('prefLoc3').value.trim();

    const error = document.getElementById("prefLocationError");

    error.textContent= "";

    if(loc1===""||loc2===""||loc3===""){
        error.textContent = "Enter all 3 preferred locations";
        return false;
    }
    if(loc1===loc2||loc2===loc3||loc1===loc3){
        error.textContent = "Preferred locations must be different";
        return false;
    }

    const pattern = /^[A-Za-z\s]+$/;

    if(!pattern.test(loc1)||!pattern.test(loc2)||!pattern.test(loc3)){
        error.textContent ="Invalid location name";
        return false;
    }
    return true;
}
...
function getPreferredLocations() {
    ...
    return[{location:loc1, priority:1},
        {location:loc2, priority:2},
        {location:loc3, priority:3}
    ];
}
...
function validateForm() {
    ...
    if (!validateLanguage()) valid = false;
    if (!validateTech()) valid = false;
    if (!validatePreferredLocation()) valid = false;
